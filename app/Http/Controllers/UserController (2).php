...
class UserController extends Controller
{
    ...
    public function update_data_operator_details(Request $request)
    {
        $user = User::findOrFail(Auth::user()->id);
        $user->data_operator_id = $request->data_operator_id;
        $user->phone_number = $request->phone_number;
        $user->save();
        return back()->with('success', 'Data Operator Updated Successfully.');
    }
    ...
    public function creditReferralAmount(Request $request)
    {
        ...
        if (!$plan->isEmpty()) {
            ...
            $user->update(
                [
                    'profit' => $user->profit + $plan[0]->fb_share_amount
                ]
            );
            DB::table('trending_user')->insert([
                'user_id' => $user->id,
    ...
    public function startMine()
    {
        ...
        // return $profit;
        if (!empty($plan) && empty($profit)) {
            $profit = new Profits();
            $profit->user_id = $user->id;
            $profit->plan_id = $plan->id;
            $profit->amount = $user->balance;
            $profit->status = 0;
            $profit->trx = $random_string;
            if ($plan->id === 10) {
                $profit->profit = $plan->percent * $plan->upgrade / 100 * 24;
                $profit->end_date = Carbon::now()->addDay();
            } else {
                $profit->profit = $plan->percent * $plan->upgrade / 100;
                $profit->end_date = Carbon::now()->addHour();
            }
            // $profit->end_date = Carbon::now()->addSecond(9);
    ...
    public function endMine()
    {
        $user = auth()->user();
        $profit = Profits::where('user_id', $user->id)->where('status', 0)->latest('id')->first();
        $user_profit = $user->balance + $profit->profit;
        $profit->update(['status' => 1]);
        $user = User::find($user->id);
        $user->update(['balance' => $user_profit]);
        return array('status' => '1');
        // }
    }
    public function latest_sponsored_post()
    {
        ...
        $data['post'] = Blog::where('cat_id', 2)->where('post_date', Carbon::now()->format('Y-m-d'))->latest()->first();
        // return $data['post'];
